...
#classesy= ['bookshelf',  'cabinet',  'chair',  'lamp',  'monitor',  'sofa',  'table']

for c in classesy:
    pointnet = PointNet()
    pointnet.load_state_dict(torch.load('/home/ubuntu/Pointnet/trained/July03/experiment_04/intraclass/'+c+'/save_49.pth'))
    pointnet.eval();

    path= '/home/ubuntu/3d-point-clouds-HyperCloud/experiments_arpit/all_intra_class_multiple_epoch_dataset100/'
    print (c)
    results=np.zeros((100,150,3,100))
    with torch.no_grad():
        for category in range(100): 
            ...
            for epoch in range(2,302,2): #### 1350 pcs available
                ...
                count+=1
    np.save('/home/ubuntu/Pointnet/results/July03/experiment_04/intraclass/'+c+'.npy', results)
