...
def load_test_data(path, category, epoch, example):
    if len(str(epoch))>4:
      n= str(epoch)
    else:
    ...
    coordinates = torch.load(path+'bathtub_'+y+'/coordinates/bathtub_'+y+'_'+n+'_'+str(example)+'_'+'recon_coord.pt')
...
pointnet = PointNet()
pointnet.load_state_dict(torch.load('/home/ubuntu/Pointnet/trained/shapenetPointnet10/save_49.pth'))
pointnet.eval();


path= '/home/ubuntu/3d-point-clouds-HyperCloud/experiments_arpit/all_intra_class_multiple_epoch_dataset100/bathtub/'
    ...
    for category in range(100): 
        ...
        for epoch in range(2,302,2): #### 1350 pcs available
            ...
            for example in range(3):
                inputs, labels = load_test_data(path, category, epoch,example)
...
np.save('/home/ubuntu/Pointnet/results/100bathtubs3reconstructionsAllCheckShapenet.npy', results)